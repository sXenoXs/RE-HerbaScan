...
void main() {
  group('Real GradCAM Tests', () {
    test('should generate GradCAM with real plant image', () async {
      ...
      print(
          '🔄 Generating GradCAM for Sambong (confidence: ${(predictions[targetClassIndex] * 100).toStringAsFixed(1)}%)...');
    ...
    test('should generate multiple GradCAMs for top predictions', () async {
      ...
      print(
          '🏆 Top predictions: ${topIndices.map((i) => 'Class $i (${(predictions[i] * 100).toStringAsFixed(1)}%)').join(', ')}');
    ...
    test('should handle different plant species', () async {
      ...
      for (final testCase in testCases) {
        ...
        print(
            '🔄 Testing ${testCase['name']} (${((testCase['confidence'] as double) * 100).toStringAsFixed(1)}%)...');
