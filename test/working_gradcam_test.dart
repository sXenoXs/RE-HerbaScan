...
void main() {
  group('Working GradCAM Tests', () {
    ...
    test('should handle different attention patterns', () async {
      ...
      final patterns = [
        {
          'name': 'High Attention Center',
          'pattern': _createCenterAttentionPattern()
        },
        {
          'name': 'High Attention Edges',
          'pattern': _createEdgeAttentionPattern()
        },
        {
          'name': 'Random Attention',
          'pattern': _createRandomAttentionPattern()
        },
      ];
      ...
      for (final pattern in patterns) {
        ...
        final directory = await Directory.systemTemp.create();
        final fileName =
            '${pattern['name'].toString().toLowerCase().replaceAll(' ', '_')}_gradcam.png';
  ...
  for (int y = 0; y < size; y++) {
    for (int x = 0; x < size; x++) {
      ...
      final distance =
          sqrt((x - centerX) * (x - centerX) + (y - centerY) * (y - centerY));
      final maxDistance =
          sqrt((size / 2) * (size / 2) + (size / 2) * (size / 2));
